...
#define MAX_IMAGE_SIZE 36865  // 256x288 pixels

// Global variable for the image buffer
...
void saveImageToBuffer()
{
    ...
    ret = fp->takeImage();
    if (ret == R503_OK) {
        Serial.println("Finger captured. Downloading image...");
        
        uint16_t imageSize = MAX_IMAGE_SIZE;
        
        Serial.println("Before downloadImage call");
        ret = fp->downloadImage(imageBuffer, imageSize);
        Serial.println("After downloadImage call");
        
        if (ret == R503_OK) {
            Serial.println("Image downloaded successfully");
            Serial.printf("Image size: %u\n", imageSize);
            
            // Send the image data over Serial (limited to first 1000 bytes for safety)
            uint16_t sendSize = min(imageSize, (uint16_t)1000);
            Serial.write(imageBuffer, sendSize);
            
            fp->setAuraLED(aLEDBreathing, aLEDGreen, 255, 1);
            Serial.println("\nFirst 100 bytes of image data (hex):");
            for (uint16_t i = 0; i < min(imageSize, (uint16_t)100); i++) {
                Serial.printf("%02X ", imageBuffer[i]);
                if ((i + 1) % 16 == 0) Serial.println();
            }
        } else {
            Serial.printf("Failed to download image (code: 0x%02X)\n", ret);
            fp->setAuraLED(aLEDFlash, aLEDRed, 50, 3);
        }
    } else {
        Serial.printf("Failed to capture finger (code: 0x%02X)\n", ret);
        fp->setAuraLED(aLEDFlash, aLEDRed, 50, 3);
    }
}